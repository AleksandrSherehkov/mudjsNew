...
$('.trigger').on('text', function(e, text) {
    if (text && text.match && text.match('ВЫБИЛ.? у тебя оружие, и оно упало на землю!$')) {
//        echo('>>> Подбираю оружие с пола, очистив буфер команд.\n');
//        send('\\');
//        send('взять ' + weapon + '|надеть ' + weapon);
    }

    if (text && text.match && text.match('^Ты умираешь от голода|^Ты умираешь от жажды')) {
...
//        send('положить боч сумка');
    ...
    if (text && text.match && text.match('Обессилев, ты падаешь лицом вниз!')) {
//        echo('>>> ЕЩЕ РАЗОК!!!\n');
