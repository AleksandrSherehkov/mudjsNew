...
function echo(txt) {
  ...
  if (txt.length !== 0) {
    ...
    if (terminal) {
      terminal.dispatchEvent(new CustomEvent('output-html', { 
        detail: [output.outerHTML] 
      }));
    }
  } else {
    const terminal = document.querySelector('.terminal');
    if (terminal) {
      terminal.dispatchEvent(new CustomEvent('output', { 
        detail: '\n' 
      }));
    }
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (triggers) {
    triggers.addEventListener('input', function (e) {
      // Check if the event has detail property (custom event)
